...
public class IMDB2DB
{
	...
	public static void main(String[] args)
	{
		...
		converter.readActorsList();
		converter.readActressesList();
	...
	private void initDatabase()
	{
	  ...
	  {		
			...
			selectMovieIDNULL=db.prepareStatement("SELECT MovieID FROM Movies WHERE title=? and year=? and addition IS NULL and type=? and episode=?");		
			...
			selectAlternativeMovieIDNULL=db.prepareStatement("SELECT MovieID FROM AlternativeTitles WHERE alternativetitle=? and year=? and addition IS NULL and type=? and episode=?");		
			...
			insertPerson=db.prepareStatement("INSERT INTO Persons VALUES (?,?,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL)");
...
//			db.execUpdate("CREATE TABLE AlternativeTitles(MovieID number(10), AlternativeTitle varchar(500), Year Number(30), addition varchar(10), Type Number(1), Episode VARCHAR2(500), Commentary VARCHAR(300))");
			...
			db.execUpdate("CREATE TABLE Taglines(MovieID number(10), tagline varchar(500))");
			...
			db.execUpdate("CREATE TABLE Certificates(MovieID number(10), certificate varchar(100))");
			db.execUpdate("CREATE TABLE Countries(MovieID number(10), country varchar(100), position number(2))");
			
			db.execUpdate("CREATE TABLE FullBiographies(PersonID number(10), Author VARCHAR(100), Biography LONG)");
			...
			db.execUpdate("CREATE TABLE Pictorials(PersonID number(10), Pictorial varchar(500))");
...
//		System.out.println("demangleComplexTitle("+complexTitle+")");
		...
		if (idx!=-1)
		{
			...
			complexTitle=complexTitle.substring(0,idx);
		}
		int year=-1;
...
//					System.out.println("type=#"+t.type+"#");
	...
	private void readActorsList()
	{
		System.out.println("import actors.");
		...
		try
		{      
			BufferedReader buffer = new BufferedReader(new InputStreamReader(new GZIPInputStream(new FileInputStream(DIR+"actors.list.gz"))));
			String inputStr;
			...
			while ((inputStr=buffer.readLine()) != null)
			{
				...
				else if ( inputStr.startsWith("THE ACTORS LIST")) 
				{
					inActor = true ; 
					buffer.readLine(); // skip "==========="
					buffer.readLine(); // skip blank line
				...
				else if (inActor)
				{
					...
					insertActsInMovie.setString(3,role);
					insertActsInMovie.setString(4,position);
						
					insertActsInMovie.execute();
				}
		...
		System.out.println("imported "+actorcnt+" actors.");
		System.out.println("imported "+(totalcnt-ignoredcnt)+" movies, ignored "+ignoredcnt+".");
	}
	
	private void readActressesList()
	{
		System.out.println("import actresses.");
		...
		try
		{      
			BufferedReader buffer = new BufferedReader(new InputStreamReader(new GZIPInputStream(new FileInputStream(DIR+"actresses.list.gz"))));
			String inputStr;
			...
			while ((inputStr=buffer.readLine()) != null)
			{
				...
				else if ( inputStr.startsWith("THE ACTRESSES LIST")) 
				{
					inActor = true ; 
					buffer.readLine(); // skip "==========="
					buffer.readLine(); // skip blank line
				...
				else if (inActor)
				{
					...
					inputStr=inputStr.trim();
					int lessthan=inputStr.lastIndexOf("<");
					int greatherthan=inputStr.lastIndexOf(">");
					
					
					if (lessthan!=-1)
					{
						position=inputStr.substring(lessthan+1,greatherthan);
						inputStr=inputStr.substring(0,lessthan).trim();
					}
					
					int squarebracketopen=inputStr.lastIndexOf("[");
					int squarebracketclose=inputStr.lastIndexOf("]");
					...
					int parenthesisclose=inputStr.lastIndexOf(')');
					if (parenthesisclose!=-1) title=findMovie(inputStr.substring(tab,Math.min(parenthesisclose+1,inputStr.length())).trim());
		...
		{
			System.out.println("readActressList(): "+e);
			e.printStackTrace();
			System.exit(1);
		}
		
		System.out.println("imported "+actorcnt+" actresses.");
		System.out.println("imported "+(totalcnt-ignoredcnt)+" movies, ignored "+ignoredcnt+".");
	}
		...
		{      
			...
			while ((inputStr=buffer.readLine()) != null)
			{
				...
				else if (inActor)
				{
					...
					{
						as=inputStr.substring(paropen+1,inputStr.length()-1);
	...
	private void readCostumeDesignersList()
	{
		...
		try
		{      
			BufferedReader buffer = new BufferedReader(new InputStreamReader(new GZIPInputStream(new FileInputStream(DIR+"costume-designersctors.list.gz"))));
			String inputStr;
	...
	private void readAlternativeNames()
	{
		...
		try
		{      
			BufferedReader buffer = new BufferedReader(new InputStreamReader(new GZIPInputStream(new FileInputStream(DIR+"aka-names-list.list.gz"))));
			String inputStr;
									...
									if (t!=null)
									{
										if (t.addition!=null)
										{
											...
											if (t.episode!=null)
												...
												selectMovieID.setNull(5,Types.VARCHAR);
										...
										else
										{
											...
											if (t.episode!=null)
												selectMovieIDNULL.setString(4, t.episode);
											else
												selectMovieIDNULL.setNull(4,Types.VARCHAR);
				...
				if (movieID==-1) // did not found it, so look at alternative titles
				{
					if (t.addition!=null)
					{
						...
						if (t.episode!=null)
							...
							selectAlternativeMovieID.setNull(5,Types.VARCHAR);
					...
					else
					{
						...
						if (t.episode!=null)
							selectAlternativeMovieIDNULL.setString(4, t.episode);
						else
							selectAlternativeMovieIDNULL.setNull(4,Types.VARCHAR);
