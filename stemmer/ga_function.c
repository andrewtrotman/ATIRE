...
double GA_function::call() {
    ...
    for (i = 0; i < query_count; i++) {
        sum_of_average_precisions += function(search_engine, all_queries[i], &hits, stemmer, topic_ids[i], map);
    }
    return sum_of_average_precisions / (double) (query_count);
}

GA_function::GA_function(double (*function) (ANT_search_engine *, char *, long *, ANT_stemmer *, long, ANT_mean_average_precision *),
