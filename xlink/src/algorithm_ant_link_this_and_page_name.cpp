...
void algorithm_ant_link_this_and_page_name::load_names()
{
	if (!config_)
		return;
...
void algorithm_ant_link_this_and_page_name::search_anchor_by_page_nam(links* lx, char **term_list, const char *source)
{
	char **first, **last;
	char *where_to;
...
//wiki_entry_array *algorithm_ant_link_this_and_page_name::find_page(char *name)
//{
//	std::map<std::string, wiki_entry_array>::iterator iter = names_map_.find(name);
//	return iter == names_map_.end() ? NULL : &(iter->second);
//}

//void algorithm_ant_link_this_and_page_name::create_posting(wiki_entry_array& wea, link* lnk)
//{
