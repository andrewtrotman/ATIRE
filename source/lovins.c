 ...
 *
 * Modified by Eibe Frank (eibe@cs.waikato.ac.nz) in Jan 2001 to be 
 * closer to Lovins' original description. The three
 * resulting changes are marked with CHANGED.
 *
 * Modified by Andrew Trotman (andrew@cs.otago.ac.nz) March 2009 for
 * integration into ANT.  Changes are marked with ASPT
 *
...
static char *RCSID = "$Id: stem.c,v 1.2 1994/09/20 04:20:44 tes Exp $";

/*
	ASPT:
	sysfuncs is full of all sorts of complete rubbish that is irrelevant
	and so it is removed and replaced with what it should have read
*/
#ifdef NEVER
	#include "sysfuncs.h"
#else
...
static int 
BB (stem_length, end) /* CHANGED */
     int stem_length;
     char *end;
{
  if ((stem_length > 2) && (strncmp (end - 2, "met", 3) != 0) &&
      ((stem_length < 4) || (strncmp (end - 3, "ryst", 4) != 0))) 
    return 1;
  else
    return 0;
}
...
static void				// ASPT: made this function static and added (above) a new public function to call it
stem (u_char * word)
{
