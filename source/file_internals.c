...
*/
	...
	while (bytes_to_read > chunk_size)
		{
		if (ReadFile(fp, destination, chunk_size, &got_in_one_read, NULL) == 0)
			return 0;
		bytes_to_read -= chunk_size;
		}
	if (bytes_to_read > 0)			// catches a call to read 0 bytes 
		if (ReadFile(fp, destination, (DWORD)bytes_to_read, &got_in_one_read, NULL) == 0)
			return 0;
...
*/
	...
	while (bytes_to_write > chunk_size)
		{
		if (WriteFile(fp, destination, chunk_size, &written, NULL) == 0)
			return 0;
		bytes_to_write -= chunk_size;
		}
	if (bytes_to_write > 0)			// catches a call to read 0 bytes 
		if (WriteFile(fp, destination, (DWORD)bytes_to_write, &written, NULL) == 0)
			return 0;
