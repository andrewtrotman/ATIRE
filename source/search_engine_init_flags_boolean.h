...
public:
	...
	void init(long long documents)
		{
		ANT_bitstring::set_length(length = documents);
		rewind();
		}

	void set_length(long long new_length) { length = new_length; ANT_bitstring::unsafe_set_length(new_length); }
	void resize(long long new_length) { length = new_length; ANT_bitstring::unsafe_set_length(new_length); }
